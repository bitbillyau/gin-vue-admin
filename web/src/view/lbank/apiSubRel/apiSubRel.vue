  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column type="selection" width="55" />
            ...
            <el-table-column align="left" label="子账户编号" prop="marketuNid" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateApiSubRelFunc(scope.row)">编辑</el-button>
    ...
    {{ detailFrom.apiId }}
                    ...
                    <el-descriptions-item label="子账户编号">
    {{ detailFrom.marketuNid }}
</el-descriptions-item>
...
const getTableData = async() => {
  const table = await getApiSubRelList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteApiSubRelByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateApiSubRelFunc = async(row) => {
    const res = await findApiSubRel({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteApiSubRelFunc = async (row) => {
    const res = await deleteApiSubRel({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
